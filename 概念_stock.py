...
data_csv = pd.read_csv("C:/Users/13115925968/OneDrive/桌面/同花顺/all_data(1).csv", encoding='gb18030')
...
async def main():
    for i in data_csv['链接'].dropna(axis=0):
        symbol = i[-7:-1]
        browser = await launch(OPTIONS)
        page = await browser.newPage()
        await page_evaluate(page)
        await page.goto("http://q.10jqka.com.cn/gn/detail/code/{}/".format(symbol))
        ...
        while True:
            try:
                # 点击进入下一页
                text = await page.querySelectorAllEval('#m-page', 'nodes => nodes.map(node => node.innerText)')
                if "下一页" not in text[0]:
                    break
        ...
        df = pd.DataFrame(results, columns=["序号", "代码", "名称", "现价", "涨跌幅(%)", "涨跌", "涨速(%)", "换手(%)", "量比",
                                            "振幅(%)", "成交额", "流通股", "流通市值", "市盈率", "加自选", "链接", "链接(1)", "其他"])
        df.to_csv("C:/Users/13115925968/OneDrive/桌面/同花顺/单个概念/{}.csv".format(symbol))
        await asyncio.sleep(3)
